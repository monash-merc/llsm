...
case class FileMetadata(filename: FilenameMetadata,
...
object FileMetadata extends MetadataImplicits with Ordering[FileMetadata] {
  ...
  def extractMetadata(dir: Path, config: ConfigurableMetadata)
    : Either[MetadataError, List[FileMetadata]] = {
    val files: List[Path] =
      Files.list(dir).collect(Collectors.toList[Path]).asScala.toList
    println(files)
    files
      .filter(f =>
      ...
      case (List(), _) =>
        Left(
          MetadataIOError(s"Could not find metadata file in: ${dir.toString}"))
      case (_, List()) =>
        Left(MetadataIOError(s"No images were detected in: ${dir.toString}"))
  ...
  def readMetadataFromTxtFile(
      path: Path): Either[MetadataError, TextMetadata] = {
    ...
    wfLines.mkString("\n").split(camDelimiter).toList match {
      case List(w, c) =>
        for {
          ...
          sample <- Parser[SampleStage](
            lines
              .filter(_.startsWith("Angle between stage and bessel beam"))
              .next).left.map {
            case ParsingFailure(m, e) => MetadataIOError(s"$m:\n$e")
          }
  ...
  /**
    ...
    */
  def parseMetadataFromFileNames(imgFileList: List[Path])
    : Either[MetadataError, List[FilenameMetadata]] = {
    val fnMeta: Parser.Result[List[FilenameMetadata]] =
      sequenceListEither(
        imgFileList.map[Parser.Result[FilenameMetadata],
                        List[Parser.Result[FilenameMetadata]]](path =>
          Parser[FilenameMetadata](path.getFileName().toString)))

    fnMeta.left.map { case ParsingFailure(m, e) => MetadataIOError(s"$m:\n$e") }
  }

  def extractImgMeta(
      imgFileList: List[Path],
      textMeta: TextMetadata): Either[MetadataError, List[ImgMetadata]] = {
    val scifio: SCIFIO = new SCIFIO()
    sequenceListEither(
      imgFileList.map[Either[MetadataError, ImgMetadata],
                      List[Either[MetadataError, ImgMetadata]]](p =>
        Try {
          ...
          case Success(meta) => {
            val im = meta.get(0)
            val zInterval = Math
              .sin(Math.toRadians(textMeta.sample.angle)) * textMeta.waveform.sPZTInterval + textMeta.waveform.zPZTInterval
