...
object MetadataUtils {
  ...
  def createImageMetadata(img: LLSMImg): ImageMetadata = {
    ...
    val xAxis: CalibratedAxis =
      new DefaultLinearAxis(Axes.X, "um", fm.config.xVoxelSize)
    val yAxis: CalibratedAxis =
      new DefaultLinearAxis(Axes.Y, "um", fm.config.yVoxelSize)
    val zAxis: CalibratedAxis = new DefaultLinearAxis(
      Axes.Z,
      "um",
      Deskew.calcZInterval(waveform.sPZTInterval,
  ...
  def createImageMetadata(imgs: List[LLSMImg]): Option[ImageMetadata] = {
    ...
    val (name, channelIdxs, times) =
      imgs.foldLeft(
        (imgs(0).meta.filename.name, TreeSet.empty[Int], TreeSet.empty[Long])) {
        case ((n, c, t),
              LLSMImg(_,
                      FileMetadata(
    ...
    imgs.headOption map { refImg =>
      {
        ...
        val xAxis: CalibratedAxis =
          new DefaultLinearAxis(Axes.X, "um", config.xVoxelSize)
        val yAxis: CalibratedAxis =
          new DefaultLinearAxis(Axes.Y, "um", config.yVoxelSize)
        ...
        val axes: List[CalibratedAxis] = List(xAxis, yAxis) ++
          ...
          (if (times.size > 1) {
             ...
             val avInterval
               : Double = diffs.foldRight(0L)(_ + _).toDouble / diffs.size
