...
trait ImplicitParsers {
  ...
  implicit val wfStageParser = new Parser[WaveformMetadata.Stage] {
    def apply(s: String): Parser.Result[WaveformMetadata.Stage] = s.split("\\t").toList match {
      case List(label, off, int, pix) => for {
        ...
        p <- Either.fromTry(Try(pix.toLong)).leftMap(e => ParsingFailure("Could not parse pixel number", e))
  ...
  implicit val wfStackParser = new Parser[WaveformMetadata.Stack] {
    def apply(s: String): Parser.Result[WaveformMetadata.Stack] = s.split("\\t").toList match {
  ...
  implicit val wfExcitationParser = new Parser[WaveformMetadata.Excitation] {
    def apply(s: String): Parser.Result[WaveformMetadata.Excitation] = s.split("\\t").toList match {
  ...
  implicit val wfTypeParser = new Parser[WaveformMetadata.Type] {
    def apply(s: String): Parser.Result[WaveformMetadata.Type] = s.split("\\t").toList match {
  ...
  implicit val wfCycleParser = new Parser[WaveformMetadata.Cycle] {
    def apply(s: String): Parser.Result[WaveformMetadata.Cycle] = s.split("\\t").toList match {
  ...
  implicit val wfZMotionParser = new Parser[WaveformMetadata.ZMotion] {
    def apply(s: String): Parser.Result[WaveformMetadata.ZMotion] = s.split("\\t").toList match {
  ...
  private[this] def sequenceChannels(xGalv: List[WaveformMetadata.Stage], zGalv: List[WaveformMetadata.Stage], zPZT: List[WaveformMetadata.Stage], sPZT: List[WaveformMetadata.Stage], stack: List[WaveformMetadata.Stack], excitation: List[WaveformMetadata.Excitation]): List[WaveformMetadata.Channel] = {
    for {
      xg <- xGalv
      zg <- zGalv
      zp <- zPZT
  ...
  implicit val wfParser = new Parser[WaveformMetadata] {
    def apply(s: String): Parser.Result[WaveformMetadata] = s.split("\\n\\n").toList match {
      case List(t, xg, zg, zp, sp, stacks, ex, cycle, zmotion) => for {
        wft <- Parser[WaveformMetadata.Type](t.trim)
        xGalv <- xg.split("\\n").toList.map(l => Parser[WaveformMetadata.Stage](l)).sequenceU
        zGalv <- zg.split("\\n").toList.map(l => Parser[WaveformMetadata.Stage](l)).sequenceU
        zPZT <- zp.split("\\n").toList.map(l => Parser[WaveformMetadata.Stage](l)).sequenceU
        sPZT <- sp.split("\\n").toList.map(l => Parser[WaveformMetadata.Stage](l)).sequenceU
        st <- stacks.split("\\n").toList.map(l => Parser[WaveformMetadata.Stack](l)).sequenceU
        exp <- ex.split("\\n").toList.map(l => Parser[WaveformMetadata.Excitation](l)).sequenceU
        cyc <- Parser[WaveformMetadata.Cycle](cycle.trim)
        zm <- Parser[WaveformMetadata.ZMotion](zmotion.trim)
        } yield {
          WaveformMetadata(wft.name, sPZT(0).excitationPixels, zPZT(0).interval, sPZT(0).interval, sequenceChannels(xGalv, zGalv, zPZT, sPZT, st, exp), st(0).number, cyc.mode, zm.mode)
        }
    }
  }
  ...
  implicit val cameraParser = new Parser[CameraMetadata] {
    def apply(s: String): Parser.Result[CameraMetadata] =
      s.split("\\n").toList match {
        case l: List[String] if l.size == 31 => for {
        ...
        } yield CameraMetadata(model, serial, frameTransfer,
          trig, exposure, cycleT, cycleR, frameMode, readMode, roi, binning,
          pixels, vssSpeed, hssSpeed, vssAmp, outAmp, temp, adChannel,
          emGain, hsSpeedIndex, preampGain, bitDepth, baslineClamp, spool,
          expNumber, cropped, lRoi, rRoi, fovRoi, imgNumber, subRois)
        case _ => {
