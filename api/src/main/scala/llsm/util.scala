...
import llsm.io.metadata.{FileMetadata, MetadataUtils, WaveformMetadata}
...
import ome.units.quantity.{Length, Time}
import ome.xml.meta.MetadataStore
import ome.xml.model.enums.{AcquisitionMode, ContrastMethod, IlluminationType}
import ome.xml.model.primitives.{Color, NonNegativeInteger}
...
object ImgUtils {
  ...
  /** Generate a 5D image from a list of Imgs
    ...
    */
  def aggregateImgs(
      lImgs: List[LLSMImg]): Option[SCIFIOImgPlus[UnsignedShortType]] = {
    val aggImg: Option[Img[UnsignedShortType]] = {
      ...
      tStacks.flatMap { ts =>
        {
          val stack =
            if (ts.size > 1) Some(Views.stack[UnsignedShortType](ts.asJava))
            else ts.headOption
  ...
  /**
    ...
    *
    */
  private def getColour(wl: Double,
                        min: Double,
                        max: Double,
                        offset: Double): Int = {
  ...
  def addChannelMeta(omexml: MetadataStore): FileMetadata => Unit = {
    case FileMetadata(_,
                      WaveformMetadata(_, _, _, _, channels, _, _, _),
                      _,
                      _,
                      _) =>
      channels.foreach {
        case WaveformMetadata.Channel(_,
                                      _,
                                      _,
                                      _,
                                      _,
                                      _,
                                      WaveformMetadata.Excitation(
                                        ch,
                                        filter @ _,
                                        laser,
                                        power @ _,
          ...
          omexml.setChannelIlluminationType(IlluminationType.EPIFLUORESCENCE,
                                            0,
                                            ch)
          omexml.setChannelContrastMethod(ContrastMethod.FLUORESCENCE, 0, ch)
          omexml.setChannelColor(
            new Color(getColour(laser.toDouble, 380, 650, 70)),
            0,
            ch)
          omexml.setChannelExcitationWavelength(
            new Length(laser, UNITS.NANOMETRE),
            0,
            ch)
        }
      }
  ...
  def writeOMEMetadata[M[_]](
    ...
    val omeElem: Option[Node] = MetadataUtils
      .createImageMetadata(imgs)
      .flatMap(meta => {
              ...
              case LLSMImg(_, FileMetadata(file, wave, _, _, _)) => {
                (0 until wave.nSlices.toInt).foreach {
                  z =>
                    {
                      val i =
                        ((file.stack * wave.channels.size * wave.nSlices) +
                      ...
                      omexml.setTiffDataFirstT(
                        ...
                        0,
                        i)
                      omexml.setTiffDataPlaneCount(new NonNegativeInteger(1),
                                                   0,
                                                   i)
                      omexml.setUUIDFileName(file.name, 0, i)
                      omexml.setUUIDValue(file.id.toString, 0, i)

                      omexml.setPlaneTheC(new NonNegativeInteger(file.channel),
                                          0,
                                          i)
                      omexml.setPlaneTheZ(new NonNegativeInteger(z), 0, i)
                      omexml.setPlaneTheT(new NonNegativeInteger(file.stack),
                                          0,
                                          i)
                      wave.channels.headOption
                        .foreach(
                          c =>
                            omexml.setPlaneExposureTime(
                              new Time(c.excitation.exposure,
                                       UNITS.MILLISECOND),
                              0,
                              i))
                    }
    ...
    omeElem match {
      case Some(ome) => {
        M.catchNonFatal {
          val _ = XML.save(Paths.get(outPath.toString, companionName).toString,
                           ome,
                           "UTF-8",
                           true)
        }
      ...
      case None =>
        M.raiseError(
          new Exception("Unable to create OMEXML companion metadata object."))
    }
