...
object ImgWriterAPI {
  ...
  case class WriteImg[A](path: Path, img: LLSMImg, next: FileMetadata => A)
      extends ImgWriterF[A]

  implicit val imgWriter = new ImgWriterAPI[ImgWriterF] {
    def writeImg(path: Path, img: LLSMImg): ImgWriterF[FileMetadata] =
      WriteImg(path, img, identity)
  }

  implicit def imgWriterInject[F[_], G[_]](
      implicit F: ImgWriterAPI[F],
      I: InjectK[F, G]): ImgWriterAPI[Free[G, ?]] =
...
case class WriteOMETIFF(path: Path, img: LLSMImg)
    extends LowWriterF[FileMetadata]
