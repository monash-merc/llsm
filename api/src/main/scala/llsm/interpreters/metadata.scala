...
import llsm.algebras.{LoggingAPI, LoggingF, MetadataAPI, MetadataF, MetadataLow, MetadataLowF}
import llsm.fp._
import llsm.io.metadata.{ConfigurableMetadata, FilenameMetadata, LLSMMeta, FileMetadata, Parser, ParsingFailure}
...
trait MetadataInterpreters {
  ...
  def basicMetadataReader[M[_]](config: ConfigurableMetadata)(
    implicit
    M: ApplicativeError[M, Throwable]
  ): MetadataF ~> M =
    new (MetadataF ~> M) {
      def apply[A](fa: MetadataF[A]): M[A] =
        metaFToMetaLowF(fa).foldMap(simpleMetaLowReader[M](config)).run(())
  }
