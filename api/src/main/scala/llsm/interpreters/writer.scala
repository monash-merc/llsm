...
import io.scif.{DefaultMetadata, SCIFIO}
...
trait ImgWriterInterpreters {
  ...
  private[this] def createAffine3D(pw: Double,
                                   ph: Double,
                                   pd: Double): AffineTransform3D = {
            ...
            if (path.toString.endsWith(".h5"))
              ...
              LowWriterAPI
                .writeError(new Exception(
                  "Unsupported output format. Only .h5 and .ome.tif are supported at this time."))
                .map(next)
        }
    }

  def imgLowWriterInterpreter[M[_]](context: Context)(
      implicit M: MonadError[M, Throwable]): LowWriterF ~> M =
    new (LowWriterF ~> M) {
      def apply[B](fa: LowWriterF[B]): M[B] =
        fa match {
          case WriteOMETIFF(path,
                            limg @ LLSMImg(img,
            ...
            val (name, ext) =
              WriterUtils.splitExtension(path.getFileName.toString)
            ...
            val success = scifio
              .translator()
              .translate(new DefaultMetadata(List(imeta).asJava), meta, false)
            ...
            if (success) {
              ...
              M.map(M.catchNonFatal(saver.saveImg(writer, imgPlus, conf)))(
                _ => {
              ...
              M.raiseError(
                new Exception(
                  "Unable to translate ImageMetadata to OMEXML metadata."))
          }
          case WriteHDF5(
              path,
              LLSMImg(
                img,
                meta @ FileMetadata(file, wave, cam @ _, sample, config))) =>
            M.catchNonFatal {
              ...
              val (units, vw, vh, vd) = (
                ...
                config.xVoxelSize,
                config.yVoxelSize,
                Deskew.calcZInterval(wave.sPZTInterval,
                                     wave.zPZTInterval,
                                     sample.angle)
              )
              ...
              val mipmaps = ProposeMipmaps.proposeMipmaps(
                new BasicViewSetup(0, "", size, voxelSize))
              val transform = createAffine3D(vw, vh, vd)
              val channels: Map[Integer, BasicViewSetup] = (0 until c)
                .map(n => {
                  val setup =
                    new BasicViewSetup(n, s"channel ${n + 1}", size, voxelSize)
                  setup.setAttribute(new Channel(n + 1))
                  Integer.valueOf(n) -> setup
                })
                .toMap
              val timePoints: List[TimePoint] =
                (0 until t).map(n => new TimePoint(n)).toList
              ...
              val partition = new Partition(
                ...
                timePoints
                  .map(t =>
                    Integer.valueOf(t.getId) -> Integer.valueOf(t.getId))
                  .toMap
                  .asJava,
                channels
                  .map {
                    case (_, c) =>
                      Integer.valueOf(c.getId) -> Integer.valueOf(c.getId)
                  }
                  .toMap
                  .asJava
              )
              val cores = Runtime.getRuntime().availableProcessors()
              val numCellCreatorThreads =
                Math.min(if (cores > 1) cores - 1 else 1, 4)
              val progress = new ProgressWriterConsole

              WriteSequenceToHdf5.writeViewToHdf5PartitionFile(
                img,
                partition,
                file.stack,
                file.channel,
              ...
              val name: String =
                s"${WriterUtils.removeExtension(path.getFileName.toString)}.xml"
              val seqPath: Path = Paths.get(outputDir.toString, name)

              WriterUtils.writeSeqDescription(path,
                                              seqPath,
                                              partition,
                                              timePoints,
                                              channels,
...
object WriterUtils {
  ...
  def writeSeqDescription(hdf5Path: Path,
                          seqPath: Path,
                          partition: Partition,
    ...
    @SuppressWarnings(Array("org.wartremover.warts.Null"))
    val seq = new SequenceDescriptionMinimal(new TimePoints(timepoints.asJava),
                                             channels.asJava,
                                             hdf5Loader,
                                             null)
    ...
    val sdm = new SpimDataMinimal(seqPath.getParent.toFile,
                                  seq,
                                  new ViewRegistrations(regs.asJava))
