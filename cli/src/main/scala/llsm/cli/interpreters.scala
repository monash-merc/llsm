...
object Interpreters {
  ...
  def consoleProgressInterpreter[M[_]](
      implicit
      M: MonadError[M, Throwable]): ProgressF ~> M =
    new (ProgressF ~> M) {
      def apply[A](fa: ProgressF[A]): M[A] =
        fa match {
          case ProgressAPI.Progress(value: Int, max: Int) =>
            M.catchNonFatal(
              println(
                s"Progress ${value.toDouble / max.toDouble * 100}%"))
  ...
  def consoleLoggingInterpreter[M[_]](
    ...
    M: MonadError[M, Throwable]): LoggingF ~> M =
    new (LoggingF ~> M) {
      def apply[A](fa: LoggingF[A]): M[A] =
        fa match {
          case LoggingAPI.Info(msg) =>
            M.catchNonFatal { if (debug) println(s"INFO: $msg") }
          case LoggingAPI.InfoCause(msg, cause) =>
            M.catchNonFatal { if (debug) println(s"INFO: $msg.\n$cause") }
          case LoggingAPI.Warn(msg) =>
            M.catchNonFatal { if (debug) println(s"WARN: $msg") }
          case LoggingAPI.WarnCause(msg, cause) =>
            M.catchNonFatal { if (debug) println(s"WARN: $msg.\n$cause") }
          case LoggingAPI.Debug(msg) =>
            M.catchNonFatal { if (debug) println(s"DEBUG: $msg") }
          case LoggingAPI.DebugCause(msg, cause) =>
            M.catchNonFatal { if (debug) println(s"DEBUG: $msg.\n$cause") }
          case LoggingAPI.Error(msg) =>
            M.catchNonFatal { if (debug) println(s"ERROR: $msg") }
          case LoggingAPI.ErrorCause(msg, cause) =>
            M.catchNonFatal { if (debug) println(s"ERROR: $msg.\n$cause") }
          case LoggingAPI.Trace(msg) =>
            M.catchNonFatal { if (debug) println(s"TRACE: $msg") }
  ...
  def cliMetadataInterpreter[M[_]](
    ...
    new (MetadataF ~> M) {
      def apply[A](fa: MetadataF[A]): M[A] =
        for {
          _ <- metadataLogging(fa).unhalt
            .foldMap(consoleLoggingInterpreter[M](debug))
